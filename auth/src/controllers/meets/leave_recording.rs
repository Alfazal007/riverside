...
pub async fn leave_recording(
    req: HttpRequest,
    app_state: web::Data<AppState>,
    leave_recording_data: web::Json<crate::types::validator::join_meeting::JoinMeeting>,
) -> impl Responder {
    let validation_errors = leave_recording_data.check_validation();
    ...
    .bind(leave_recording_data.0.meet_id)
    ...
    let new_record_leave_entry = sqlx::query_as::<_, RecordEventDb>("insert into recordevents(recording_id, action, user_id, meet_id, timestamp) values($1, $2, $3,$4,$5) returning *")
        .bind(recording.unwrap().unwrap().id)
        .bind(EventAction::Left)
        .bind(user_data.user_id)
        .bind(leave_recording_data.0.meet_id)
        .bind(leave_recording_data.0.join_time)
        .fetch_optional(&app_state.db_connection_pool)
        .await;
    if new_record_leave_entry.is_err() {
        return HttpResponse::InternalServerError().json(GeneralErrorsToBeReturned {
            error: "Issue writing to the database".to_string(),
        });
    }
    if new_record_leave_entry.as_ref().unwrap().is_none() {
